 ...
 */
exports.validatePromoCode = (0, https_1.onCall)({ cors: true }, async (request) => {
    ...
    const attemptsRef = db
        ...
        .doc(uid)
        .collection("attempts");
    const cutoff = firestore_1.Timestamp.fromMillis(Date.now() - 60 * 60 * 1000);
    ...
    await attemptsRef.add({ at: firestore_1.FieldValue.serverTimestamp() });
    ...
    const result = (0, promoCode_1.applyPromo)({
        promo: promoDoc.exists ? promoDoc.data() : null,
    ...
    const reason = result.rejection && opaque.has(result.rejection)
        ? "invalid"
...
//# sourceMappingURL=validatePromoCode.js.map
