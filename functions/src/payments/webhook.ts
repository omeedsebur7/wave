...
const ZAINCASH_SECRET = defineSecret("ZAINCASH_SECRET");
const ZAINCASH_MERCHANT_ID = defineSecret("ZAINCASH_MERCHANT_ID");
const ZAINCASH_MSISDN = defineSecret("ZAINCASH_MSISDN");
const ZAINCASH_BASE_URL = defineSecret("ZAINCASH_BASE_URL");
 ...
 */
function providerFor(path: string): PaymentProvider | null {
  if (path.includes("zaincash")) {
    return new ZainCashProvider(
      ZAINCASH_MERCHANT_ID.value(),
      ZAINCASH_SECRET.value(),
      ZAINCASH_MSISDN.value(),
      ZAINCASH_BASE_URL.value()
    );
  }
  return null;
}
...
export const paymentWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (seen.exists) return; // already applied â€” ack and move on
      ...
      const current = order.get("status") as string;
      ...
      // silently accept it.
      const expected = order.get("total_minor") as number;
