...
export const enforceRetention = onSchedule(
  ...
  { schedule: "17 3 * * *", timeoutSeconds: 540 },
  async () => {
 ...
 */
async function pruneGroupByAge(
  db: Firestore,
  group: string,
  field: string,
  days: number
 ...
 */
async function eraseDeliveryDetails(
  db: Firestore
): Promise<void> {
  ...
  for (const order of pending.docs) {
    if (!terminal.has(order.get("status") as string)) continue;
    ...
    batch.update(order.ref, {
      ...
      delivery_details_pending_erasure: FieldValue.delete(),
    });
    queued += 1;
  }
 ...
 * queue is for.
 */
async function pruneReports(db: Firestore): Promise<void> {
  const mergedCutoff = Timestamp.fromMillis(
    Date.now() - RETENTION.mergedReports * 24 * 60 * 60 * 1000
  );
  const resolvedCutoff = Timestamp.fromMillis(
    Date.now() - RETENTION.resolvedReports * 24 * 60 * 60 * 1000
  );
  ...
  if (!merged.empty) {
    const batch = db.batch();
    for (const doc of merged.docs) batch.delete(doc.ref);
    await batch.commit();
  }
