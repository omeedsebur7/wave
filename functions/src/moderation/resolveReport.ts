...
export const resolveReport = onCall({ cors: true }, async (request) => {
  ...
  const { suspendedUid } = await db.runTransaction(
    async (tx) => {
      ...
      if (report.get("action") !== "pending") {
        throw new HttpsError("aborted", "Already reviewed by someone else");
      }

      const targetType = report.get("target_type") as string;
      const targetId = report.get("target_id") as string;
        ...
        if (ownerId) {
          tx.set(
            ...
            action === "accountSuspended"
              ...
              : {
                  warning_count: FieldValue.increment(1),
                  last_warned_at: FieldValue.serverTimestamp(),
                },
            { merge: true }
          );
          if (action === "accountSuspended") suspendedUid = ownerId;
        }
      }
  ...
  if (suspendedUid) {
    await getAuth().setCustomUserClaims(suspendedUid, { suspended: true });
    ...
    await getAuth().revokeRefreshTokens(suspendedUid);
  }
 ...
 */
export const liftSuspension = onCall({ cors: true }, async (request) => {
  if (request.auth?.token?.moderator !== true) {
    throw new HttpsError("permission-denied", "Moderators only");
  }
  ...
  await getAuth().setCustomUserClaims(uid, { suspended: null });
  await db.collection("users").doc(uid).set(
    {
      suspended: false,
      suspension_lifted_at: FieldValue.serverTimestamp(),
    },
    { merge: true }
  );
...
async function resolveOwner(
  ...
  switch (targetType) {
    case "reel": {
      ...
      return (doc.get("author_id") as string | undefined) ?? null;
    }
    case "product": {
      const doc = await tx.get(db.collection("products").doc(targetId));
      return (doc.get("seller_id") as string | undefined) ?? null;
    }
    case "profile":
      return targetId;
    default:
 ...
 */
export const dedupeReport = onDocumentCreated(
  "reports/{reportId}",
  async (event) => {
    const db = getFirestore();
    ...
    const targetId = data.get("target_id") as string | undefined;
    const targetType = data.get("target_type") as string | undefined;
