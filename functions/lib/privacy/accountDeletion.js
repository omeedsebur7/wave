...
exports.exportMyData = exports.processAccountDeletion = void 0;
 ...
 */
exports.processAccountDeletion = (0, firestore_1.onDocumentCreated)("deletion_requests/{uid}", async (event) => {
    ...
    await deleteQuery(db.collectionGroup("buy_now_taps").where("user_id", "==", uid));
    ...
    await deleteQuery(db.collection("reports")
        .where("reporter_id", "==", uid)
        .where("action", "in", ["pending", "merged", "dismissed"]));
    await pseudonymise(db.collection("reports").where("reporter_id", "==", uid));
    ...
    await pseudonymise(db.collection("seller_ratings").where("author_id", "==", uid));
    ...
    for (const order of asBuyer.docs) {
        const finished = TERMINAL.includes(order.get("status"));
        batch.update(order.ref, {
            buyer_id: `deleted_${uid.slice(0, 8)}`,
            buyer_name: "Deleted account",
            buyer_deleted_at: firestore_2.FieldValue.serverTimestamp(),
            ...(finished
                ? {
                    buyer_phone: firestore_2.FieldValue.delete(),
                    address_id: firestore_2.FieldValue.delete(),
                    delivery_address: firestore_2.FieldValue.delete(),
                    ...
                    delivery_location: firestore_2.FieldValue.delete(),
                }
                : {
    ...
    await event.data?.ref.update({
        status: "completed",
        completed_at: firestore_2.FieldValue.serverTimestamp(),
    });
});
/**
 * Data export (Â§7).
 ...
 */
exports.exportMyData = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError("unauthenticated", "Sign in first");
    const uid = request.auth.uid;
    const db = (0, firestore_2.getFirestore)();
    const profile = await db.collection("users").doc(uid).get();
    const [orders, reels, products, reviews, ratings, addresses] = await Promise.all([
...
async function deleteQuery(query, batchSize = 400) {
    for (;;) {
...
async function pseudonymise(query) {
    ...
    for (const doc of snap.docs) {
        batch.update(doc.ref, {
            ...
            author_avatar_url: firestore_2.FieldValue.delete(),
        });
    }
    await batch.commit();
}
async function pseudonymiseCollectionGroup(db, group, uid) {
    const snap = await db
