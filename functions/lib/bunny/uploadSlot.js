 ...
 */
exports.publishReel = (0, https_1.onCall)({ secrets: [BUNNY_API_KEY, BUNNY_LIBRARY_ID], cors: true }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError("unauthenticated", "Sign in first");
    const db = (0, firestore_1.getFirestore)();
    const uid = request.auth.uid;
    ...
    const statusRes = await fetch(`${BUNNY_API_BASE}/library/${BUNNY_LIBRARY_ID.value()}/videos/${bunnyVideoId}`, { headers: { AccessKey: BUNNY_API_KEY.value() } });
    if (!statusRes.ok) {
        ...
        throw new https_1.HttpsError("internal", `Could not confirm the video is playable (Bunny returned ${statusRes.status})`);
    }
    ...
    if (video.status !== BUNNY_STATUS_FINISHED) {
        ...
        throw new https_1.HttpsError("failed-precondition", "This video is not ready to publish yet. Wait for it to finish encoding.");
    }
