 ...
 */
export const unlinkDeletedProduct = onDocumentDeleted(
  "products/{productId}",
  async (event) => {
 ...
 */
export const unlinkWithdrawnProduct = onDocumentUpdated(
  "products/{productId}",
  async (event) => {
    ...
    const priceChanged =
      before.get("price_minor") !== after.get("price_minor") ||
      before.get("currency") !== after.get("currency");

    if (priceChanged) {
      await syncPriceToReels(
        event.params.productId,
        after.get("price_minor") as number,
        after.get("currency") as string
      );
    }
  }
);
 ...
 */
async function syncPriceToReels(
  productId: string,
  priceMinor: number,
  currency: string
): Promise<void> {
...
async function unlinkFromReels(productId: string): Promise<void> {
  const db = getFirestore();
  ...
  for (const reel of reels.docs) {
    batch.update(reel.ref, {
      ...
      linked_product_id: FieldValue.delete(),
      // The display price goes with the link. Leaving it would put a price on a
      // Reel with nothing to buy.
      linked_product_price_minor: FieldValue.delete(),
      linked_product_currency: FieldValue.delete(),
