...
export const notifyOrderStatus = onDocumentUpdated(
  ...
  async (event) => {
    ...
    const buyerId = event.data?.after.get("buyer_id") as string;
    const orderId = event.params.orderId;

    const message = customerFacingMessage(after);
    if (!message) return; // an internal-only transition
...
export const notifyNewOrder = onDocumentCreated(
  "orders/{orderId}",
  async (event) => {
    const sellerId = event.data?.get("seller_id") as string;
    if (!sellerId) return;

    const items = (event.data?.get("items") as Array<{ title: string }>) ?? [];
 ...
 */
export const notifyReelComment = onDocumentCreated(
  ...
  async (event) => {
    ...
    const authorId = reel.get("author_id") as string;
    const commenterId = event.data?.get("author_id") as string;
...
export const notifyNewRating = onDocumentCreated(
...
export const notifyNewFollower = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (followerId === sellerId) return;

    const follower = await getFirestore()
      .collection("users")
      .doc(followerId)
      .get();
...
export const notifyChatMessage = onDocumentCreated(
  ...
  async (event) => {
    ...
    const participants =
      (conversation.get("participant_ids") as string[]) ?? [];
    const senderId = event.data?.get("sender_id") as string;
    const recipientId = participants.find((id) => id !== senderId);
    if (!recipientId) return;

    const names =
      (conversation.get("participant_names") as Record<string, string>) ?? {};
    const text = String(event.data?.get("text") ?? "");
