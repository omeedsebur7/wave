...
import { DocumentReference, FieldPath, FieldValue, Firestore, getFirestore } from "firebase-admin/firestore";
 ...
 */
export const onFollowChanged = onDocumentWritten(
  "users/{uid}/following/{sellerId}",
  async (event) => {
    ...
    const followers = await db
      ...
      .where(FieldPath.documentId(), "==", sellerId)
      .count()
      .get();

    await db.collection("users").doc(sellerId).set(
      { follower_count: followers.data().count },
      { merge: true }
    );
  }
);
 ...
 */
export const onReelWritten = onDocumentWritten(
  "reels/{reelId}",
  async (event) => {
    const authorId =
      event.data?.after?.get("author_id") ?? event.data?.before?.get("author_id");
    if (!authorId) return;
...
export const onProductWritten = onDocumentWritten(
  ...
  async (event) => {
    ...
    await db.collection("users").doc(sellerId).set(
      { product_count: count.data().count },
      { merge: true }
    );
  }
);
...
export const onReviewWritten = onDocumentWritten(
  ...
  async (event) => {
    const productId =
      event.data?.after?.get("product_id") ?? event.data?.before?.get("product_id");
    if (!productId) return;
 ...
 */
export const materializeReelFunnel = onSchedule(
  ...
  // rank on it.
  { schedule: "10,40 * * * *", timeoutSeconds: 540 },
  async () => {
    const db = getFirestore();
...
async function consumeTaps(
  ...
  batch.update(reelRef, {
    buy_now_taps: FieldValue.increment(taps.size),
  });
  for (const tap of taps.docs) batch.delete(tap.ref);
  await batch.commit();
}
